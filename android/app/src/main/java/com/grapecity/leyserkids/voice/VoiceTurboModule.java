...
public class VoiceTurboModule extends NativeVoiceTurboSpec implements RecognitionListener {
  public static final String NAME = "VoiceTurbo";
  ...
  public VoiceTurboModule(ReactApplicationContext reactContext) {
    super(reactContext);
  ...
  public void isSpeechAvailable(final Callback callback) {
    final VoiceTurboModule self = this;
    Handler mainHandler = new Handler(this.reactContext.getMainLooper());
    mainHandler.post(new Runnable() {
      @Override
      public void run() {
  ...
  public void getSpeechRecognitionServices(Promise promise) {
    @SuppressLint("QueryPermissionsNeeded") final List<ResolveInfo> services = this.reactContext.getPackageManager()
